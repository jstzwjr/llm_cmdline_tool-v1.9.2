...
LlavaRuntimeOptions mllmRuntimeOpt;
...
void mllm_init(void** mllmRuntime, const std::string& yamlConfigPath) {
    ...
    configParser.parse(llmModelOpt, mllmRuntimeOpt);
...
void llm_init(void** llmRuntime, const std::string& yamlConfigPath) {
    ...
    configParser.parse(draftModelOpt, draftRuntimeOpt);
...
int main(int argc, char* argv[]) {
    ...
    for (size_t i = 0; i < numPrompt; i++) {
        ...
        std::string promptImagePath = imagePaths[i];
