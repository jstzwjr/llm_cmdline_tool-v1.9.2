...
llm_spec_dec_per_step(SpecDecContext& ctx, const TokenType inputToken, size_t& acceptNum,
                      double& meanDraftElapsed, double& meanDraftRunOnceElapsed,
                      double& meanGenDraftTokensElapsed, double& verificationElapsed,
                      double& parallelDecodingElapsed, double& genTargetTokensElapsed,
                      double& judgmentElapsed, double& adjustCacheElapsed) {
    ...
    double draftElapsed;
    double draftRunOnceElapsed = 0, genDraftTokensElapsed = 0;
    ...
    for (size_t t = 0; t < ctx.draftLength; t++) {
        ...
        draftRunOnceElapsed = timerDraftRunOnce.reset() * 1000;
        ...
        genDraftTokensElapsed = timerGenDraftTokens.reset() * 1000;
    ...
    genTargetTokensElapsed = timerGenTargetTokens.reset() * 1000;
    LOG(DEBUG) << "[Spec-Dec][Target]: Latency for Target(" << genTokenSize << "-T) + "
               << "argmax: " << genTargetTokensElapsed << " ms.";
    ...
    LOG(DEBUG) << "[Spec-Dec][Verifying]: Latency for judgment: " << judgmentElapsed << " ms.";
    ...
    if (acceptNum == ctx.draftLength) {
        // All accept: run draft model once
...
void llm_gen_response(SpecDecContext& ctx, const TokenType firstInputToken, double& genTokPerSec) {
    ...
    while (genTokCount < ctx.maxResponse && sequenceLength < maxTokenLength) {
        if (ctx.inferType == SpecDecInferType::UnionMethodV1) {
            ...
            double verificationElapsed, parallelDecodingElapsed, genTargetTokensElapsed;
            double judgmentElapsed, adjustCacheElapsed;
            auto [acceptedTokens, lastAcceptToken] = llm_spec_dec_per_step(
                ctx, outputToken, acceptNum, meanDraftElapsed, meanDraftRunOnceElapsed,
                meanGenDraftTokensElapsed, verificationElapsed, parallelDecodingElapsed,
                genTargetTokensElapsed, judgmentElapsed, adjustCacheElapsed);
    ...
    if (ctx.inferType == SpecDecInferType::UnionMethodV1) {
        ...
        std::cout << "      All-accept Rate: " << double(allAcceptNum) / specDecCount << std::endl;
        std::cout << "\n[Latency]" << std::endl;
        std::cout << "      Drafting latency:              " << totalDraftTime / specDecCount
                  << " ms" << std::endl;
        std::cout << "          Run model once latency:    " << totalDraftRunOnceTime / specDecCount
                  << " ms" << std::endl;
        ...
        std::cout << "          Gen target tokens latency: " << totalTargetTokensTime / specDecCount
                  << " ms" << std::endl;
        std::cout << "          Judgment latency:          " << totalJudgmentTime / specDecCount
                  << " ms" << std::endl;
        std::cout << "      Adjust cache latency:          " << totalAdjustCacheTime / specDecCount
                  << " ms" << std::endl;
    }
}
