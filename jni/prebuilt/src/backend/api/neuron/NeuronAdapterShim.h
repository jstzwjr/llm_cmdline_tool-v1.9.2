...
inline void* getAdapterLibraryHandle() {
    // FIXME: Use an API in libneuron_sys_util.so to determine which .so to dlopen.
    if (sHandle == nullptr) {
        sHandle = loadAdapterLibrary("libneuronservice_adapter.mtk.so");
    }
    if (sHandle == nullptr) {
...
typedef int (*NeuronExecution_setCacheFlushHint_fn)(NeuronExecution* execution, uint8_t flushHint);
...
inline int NeuronExecution_setCacheFlushHint(NeuronExecution* execution, uint8_t flushHint) {
