...
private:
    ...
    explicit DumpOptions() {
        ...
        // Get datetime identifier
        std::time_t t = std::time(nullptr);
        std::strftime(mDateStrId, sizeof(mDateStrId), "%Y%m%d-%H%M%S", std::localtime(&t));
    }
...
public:
    ...
    const char* getIdentifierStr() const { return mDateStrId; }

private:
    int mDumpLevel = kDefaultDumpLevel;
    char mDateStrId[16]; // Identifier using datetime
};
...
std::string DumpHandler::getSavePath(const std::string& fileName) const {
    const auto identifierStr = DumpOptions::instance().getIdentifierStr();
    fs::path saveDir = fs::path(kDumpDir) / identifierStr;
    switch (mDumpType) {
