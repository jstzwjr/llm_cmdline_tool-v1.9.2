...
static void loadFilePart(void* dstBuffer, const size_t dstSize, const int fileFd,
    ...
    DCHECK_EQ(bufferSize % kMemoryAlignment, 0)
        << "Copy buffer size is not " << kMemoryAlignment << " bytes aligned: " << bufferSize;

    void* copyBuffer;
    if (posix_memalign(&copyBuffer, kMemoryAlignment, bufferSize) != 0) {
        LOG(ERROR) << "Failed to allocate copy buffer: " << strerror(errno);
        *status = false;
        return;
    }
    ...
    while (totalReadSize < filePartSize) {
        ...
        DCHECK_EQ(curReadSize % kMemoryAlignment, 0)
            << "Read size is not " << kMemoryAlignment << " bytes aligned: " << curReadSize;
        DCHECK_EQ(curOffset % kMemoryAlignment, 0)
            << "Read offset is not " << kMemoryAlignment << " bytes aligned: " << curOffset;
