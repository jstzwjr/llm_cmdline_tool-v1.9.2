...
class CallbackEmbeddingProducer : public EmbeddingProducer {
private:
    using EmbLoadFunc = std::function<void*(const InputType&)>;

public:
    explicit CallbackEmbeddingProducer(const InputType& input, const size_t equivNumToken,
                                       EmbLoadFunc embLoaderCallback, const size_t singleEmbSize)
        : EmbeddingProducer(singleEmbSize, equivNumToken),
          kInput(input),
          mEmbLoaderCallback(embLoaderCallback) {}
...
private:
    char* getEmbeddingBuffer() {
        // Lazy load the embedding
        if (!mIsEmbLoaded) {
            embBuffer = mEmbLoaderCallback(kInput);
            mIsEmbLoaded = true;
        }
        return reinterpret_cast<char*>(embBuffer);
    }

private:
    const InputType kInput;
    EmbLoadFunc mEmbLoaderCallback; // Embedding callback function
    void* embBuffer = nullptr;
