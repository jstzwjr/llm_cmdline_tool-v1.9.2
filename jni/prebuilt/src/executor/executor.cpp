...
void Executor::updateModelIO() {
    ...
    if (numInputs != getNumInputs()) {
        LOG(ERROR) << "updateModelIO: Expected num inputs (" << getNumInputs()
                   << ") != actual num inputs (" << numInputs << ").";
    }
    if (numOutputs != getNumOutputs()) {
        LOG(ERROR) << "updateModelIO: Expected num outputs (" << getNumOutputs()
                   << ") != actual num outputs (" << numOutputs << ").";
    }
    auto& allocator = getAllocator();
    for (size_t inputIdx = 0; inputIdx < numInputs; inputIdx++) {
        auto& input = getInput(inputIdx);
        auto& sizeAllocated = input.sizeBytes;
        auto& sizeRequired = input.usedSizeBytes;
        const auto before = sizeRequired;

        sizeRequired = getRuntimeInputSizeBytes(inputIdx); // Update
        if (!input.isAllocated()) {
            input.sizeBytes = sizeRequired;
            if (!allocator.allocateMemory(input)) {
                LOG(ERROR) << "Failed to allocate memory for input[" << inputIdx << "]";
            } else {
                LOG(DEBUG) << "updateModelIO: allocated input[" << inputIdx
                           << "] with size: " << sizeRequired;
            }
            continue;
        }
...
void Executor::setNumOutputs(const size_t numOutputs) {
    ...
    setNumGenericOutputs(numOutputs);
}

// Either assign buffer pointer or copy buffer content.
void Executor::setModelInput(const IOBuffer& buffer, const size_t index) {
    ...
    CHECK_LT(index, mInputs.size()) << "setModelInput index out of bound.";
    auto& curBuffer = getInput(index);
...
void Executor::initModelIOInfo() {
    ...
    if (getNumInputs() == 0) {
        ...
        const auto& expectedNumInputs = getNumInputs();
        const auto& actualNumInputs = numInputs;
        CHECK_EQ(expectedNumInputs, actualNumInputs)
            << "Number of inputs do not match, please ensure the model is correct.";
    }
    LOG(DEBUG) << "numInputs = " << numInputs;
    for (size_t inputIdx = 0; inputIdx < numInputs; inputIdx++) {
        size_t inputSize = getRuntimeInputSizeBytes(inputIdx);
        auto& input = mInputs.at(inputIdx);
    ...
    if (getNumOutputs() == 0) {
        ...
        const auto& expectedNumOutputs = getNumOutputs();
        const auto& actualNumOutputs = numOutputs;
        CHECK_EQ(expectedNumOutputs, actualNumOutputs)
            << "Number of outputs do not match, please ensure the model is correct.";
    }
    LOG(DEBUG) << "numOutputs = " << numOutputs;
    for (size_t outputIdx = 0; outputIdx < numOutputs; outputIdx++) {
        size_t outputSize = getRuntimeOutputSizeBytes(outputIdx);
        auto& output = mOutputs.at(outputIdx);
