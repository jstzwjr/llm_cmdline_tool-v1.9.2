...
void LlmRingBufferExecutor::runInferenceEpilogue() {
    ...
    if (mCacheUpdateMode == CacheUpdateMode::Copy) {
        // Run cache eviction if there exists one that triggers BEFORE cache update.
        const auto cacheShiftOffset = runCacheEviction(/*isCacheUpdated*/ false);
        mRingBufferCacheManager.advanceRingOffset(cacheShiftOffset);
    ...
    if (!isCacheEmpty) {
        mRingBufferCacheManager.advanceRingOffset(getValidModelNumInputToken());
    }
    LlmExecutor::runInferenceEpilogue();

    // Run cache eviction if there exists one that triggers AFTER cache update.
    const auto cacheShiftOffset = runCacheEviction(/*isCacheUpdated*/ true);
    mRingBufferCacheManager.advanceRingOffset(cacheShiftOffset);
}
...
void LlmRingBufferExecutor::resetTokenIndex(const size_t tokenIndex) {
    LlmExecutor::resetTokenIndex(tokenIndex);
    mRingBufferCacheManager.resetRingOffset();
}
...
void LlmRingBufferExecutor::reconstructCaches(const size_t oldCacheLength,
                                              const size_t newCacheLength) {
    ...
    if (ringOffset == 0) {
        return LlmExecutor::reconstructCaches(oldCacheLength, newCacheLength);
    }
    ...
    LlmExecutor::reconstructCaches(oldCacheLength, newCacheLength);
...
void LlmRingBufferExecutor::assignBufferSizesToMax() {
    ...
    IoBufferInflator ioBufferInflator(
        this->kRuntimeInfos, RuntimeInfo{{}, curBatchSize, curTokenSize, curCacheSize});
