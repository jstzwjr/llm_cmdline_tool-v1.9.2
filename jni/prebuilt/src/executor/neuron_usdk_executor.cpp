...
void NeuronUsdkExecutor::createNeuronMemory(IOBuffer& ioBuf) {
    ...
    if (ioBuf.ahwbHandle != nullptr) {
        ...
        CHECK_NP_ERROR(mLib->NeuronMemory_createFromFd(
...
bool NeuronUsdkExecutor::loadDla(const void* buffer, const size_t size, UsdkRuntime* runtime) {
    ...
    auto getDlaExtOperandType = [this] {
        ...
        CHECK_NP_ERROR(mLib->Neuron_getVersion(&version));
    ...
    CHECK_NP_ERROR(mLib->NeuronCompilation_setPriority(compilation, NEURON_PRIORITY_HIGH));
    CHECK_NP_ERROR(
    ...
    if (mLib->NeuronExecution_setCacheFlushHint(
            execution,
            NEURON_CACHE_FLUSH_DISABLE_SYNC_INPUT | NEURON_CACHE_FLUSH_DISABLE_INVALIDATE_OUTPUT)
        != NEURON_NO_ERROR) {
...
void NeuronUsdkExecutor::runInferenceImpl() {
    ...
    CHECK_NP_ERROR(mLib->NeuronExecution_compute(getUsdkRuntime()->execution));
}
...
#define NEURON_USDK_SET_INPUT(inputIdx, ioBuf, size)                                               \
    CHECK_NP_ERROR(mLib->NeuronExecution_setInputFromMemory(getUsdkRuntime()->execution, inputIdx, \
                                                            (NeuronOperandType*)NULL,              \
                                                            ioBuf.neuronMemory, 0, size));
...
#define NEURON_USDK_SET_OUTPUT(outputIdx, ioBuf, size)                                            \
    CHECK_NP_ERROR(mLib->NeuronExecution_setOutputFromMemory(getUsdkRuntime()->execution,         \
                                                             outputIdx, (NeuronOperandType*)NULL, \
                                                             ioBuf.neuronMemory, 0, size));
...
void NeuronUsdkExecutor::setRuntimeOffsetedInput(const size_t index, const size_t offset) {
    ...
    CHECK_NP_ERROR(mLib->NeuronExecution_setInputFromMemory(
        getUsdkRuntime()->execution, index, (NeuronOperandType*)NULL, ioBuf.neuronMemory, offset,
        ioBuf.usedSizeBytes));
}
...
void NeuronUsdkExecutor::setRuntimeOffsetedOutput(const size_t index, const size_t offset) {
    ...
    CHECK_NP_ERROR(mLib->NeuronExecution_setOutputFromMemory(
        getUsdkRuntime()->execution, index, (NeuronOperandType*)NULL, ioBuf.neuronMemory, offset,
        ioBuf.usedSizeBytes));
}
...
size_t NeuronUsdkExecutor::getRuntimeInputSizeBytes(const size_t index) const {
    ...
    auto compilation = getUsdkRuntime()->compilation;
    CHECK_NP_ERROR(mLib->NeuronCompilation_getInputPaddedSize(compilation, index, &inputSizeBytes));

    // clang-format off
    uint32_t dims[4];
    CHECK_NP_ERROR(mLib->NeuronCompilation_getInputPaddedDimensions(compilation, index, dims));
...
size_t NeuronUsdkExecutor::getRuntimeOutputSizeBytes(const size_t index) const {
    ...
    // clang-format off
    uint32_t dims[4];
    CHECK_NP_ERROR(mLib->NeuronCompilation_getOutputPaddedDimensions(compilation, index, dims));
...
void NeuronUsdkExecutor::getRuntimeInputShape(const size_t index, uint32_t* shape) const {
    ...
    CHECK_NP_ERROR(mLib->NeuronCompilation_getInputPaddedDimensions(
...
void NeuronUsdkExecutor::getRuntimeOutputShape(const size_t index, uint32_t* shape) const {
    uint32_t tmpShape[kDimensionSize];
    CHECK_NP_ERROR(mLib->NeuronCompilation_getOutputPaddedDimensions(
        getUsdkRuntime()->compilation, index, tmpShape));
    std::memcpy(shape, tmpShape, sizeof(tmpShape));
}
