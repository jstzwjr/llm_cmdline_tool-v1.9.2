...
SharedWeightsHandle::SharedWeightsHandle(const std::vector<FileSource>& sharedWeightsFiles,
                                         const size_t numDlaChunks, const bool isLastForLmHead)
    : kSharedWeightsFiles(sharedWeightsFiles),
      kNumDlaChunks(numDlaChunks),
      kIsLastForLmHead(isLastForLmHead) {
...
size_t SharedWeightsHandle::getNumSharedWeights() const {
    return kSharedWeightsFiles.size();
}

size_t SharedWeightsHandle::getNumSharedWeightsNonLmHead() const {
    return kSharedWeightsFiles.size() - static_cast<int>(kIsLastForLmHead);
}
...
void SharedWeightsHandle::setPreloadSubset(const std::unordered_set<size_t>& subsetIndexes,
    ...
    const auto numSwFiles = getNumSharedWeightsNonLmHead();
    ...
    if (kIsLastForLmHead) {
        // Currently LM Head shared weights is always set to preload when `repeatAllChunks` is true.
        mPreloadIndexes.insert(numSwFiles);
    }
}
...
void SharedWeightsHandle::preload(const bool async) {
    ...
    const auto numSwFiles = getNumSharedWeights();
    ...
    if (async) {
        ...
        mThreadPool.wait(); // Wait for buffer allocations only
...
void SharedWeightsHandle::preloadUnion(
    ...
    auto getRawPointers = [](const auto& uniquePtrs) -> std::vector<SharedWeightsHandle*> {
...
void SharedWeightsHandle::preloadUnion(const std::vector<SharedWeightsHandle*>& preloadHandles,
    ...
    for (const auto refHandle : refHandles) {
        ...
        for (size_t swIdx = 0; swIdx < swFiles.size(); swIdx++) {
            if (refHandle->isPreloaded(swIdx)) {
                preloadFilesMap.try_emplace(swFiles[swIdx], std::pair{refHandle, swIdx});
            }
        }
    }
    ...
    for (size_t swHandleIdx = 0; swHandleIdx < preloadHandles.size(); swHandleIdx++) {
        ...
        for (const auto preloadIdx : preloadIdxs) {
            ...
            preloadFilesMap.emplace(swFile, std::pair{swHandle, preloadIdx});
        }
    ...
    auto getPreloadedSwBuffer = [&](const FileSource& swFile) {
        ...
        const auto& [preloadHandle, swIdx] = preloadFilesMap[swFile];
    ...
    for (size_t swHandleIdx = 0; swHandleIdx < preloadHandles.size(); swHandleIdx++) {
        ...
        if (preloadIdxs.empty()) {
            ...
            const auto numSwFiles = swHandle->getNumSharedWeights();
...
void SharedWeightsHandle::wait() const {
    // The caller should have finished calling preload APIs, so there is no use for thread pool
    // until the next call to preload APIs happens. In that case, we can just join the threads.
    mThreadPool.joinAll();
}
...
SharedWeights SharedWeightsHandle::getLmHeadSharedWeights() const {
    if (!kIsLastForLmHead) {
        LOG(ERROR) << "SharedWeightsHandle has no shared weights for LM Head.";
        return {};
    }

    const bool preloaded = !mSharedWeightsBuffers.empty();

    SharedWeights lmHeadSw;
    auto& lmHeadSwFiles = lmHeadSw.files; // Always assigned regardless preloaded or not
    auto& lmHeadSwBuffers = lmHeadSw.buffers;

    lmHeadSwFiles.reserve(1);
    if (preloaded) {
        lmHeadSwBuffers.reserve(1);
    }

    const size_t lmHeadSwIdx = getNumSharedWeights() - 1;
    lmHeadSwFiles.push_back(kSharedWeightsFiles[lmHeadSwIdx]);
    if (preloaded) {
        auto& swBuffer = mSharedWeightsBuffers[lmHeadSwIdx];
        lmHeadSwBuffers.push_back(swBuffer ? swBuffer->getIOBuffer() : IOBuffer());
    }
    return lmHeadSw;
}

bool SharedWeightsHandle::hasLmHead() const {
    return kIsLastForLmHead;
}
