...
public:
    explicit SharedWeightsHandle(const std::vector<FileSource>& sharedWeightsFiles,
                                 const size_t numDlaChunks = 1, const bool isLastForLmHead = false);
    ...
    SharedWeights getLmHeadSharedWeights() const;

    bool hasLmHead() const;

private:
    size_t getNumSharedWeights() const;

    size_t getNumSharedWeightsNonLmHead() const;

private:
    const size_t kNumDlaChunks;
    const bool kIsLastForLmHead; // Whether the last shared weights is for LM Head
