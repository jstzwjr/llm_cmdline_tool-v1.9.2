...
void RotaryEmbeddingMasterLut::setEmbed(void* rotEmbQCosBuffer, void* rotEmbQSinBuffer,
    ...
    const auto padLength = leftPadLength + rightPadLength;
    const size_t numCachedTokens = std::min(tokenIndex, modelCacheSize);

    //===----------------------------------------===//
    // RotEmb for Query: [modelTokenSize, kHeadDim]
    //===----------------------------------------===//

    const auto startTokenIndexQ = numCachedTokens;
    ...
    const size_t numKLookup =
        std::min(modelTokenSize + modelCacheSize, modelTokenSize + numCachedTokens - padLength);

    const auto leftPadLengthK = modelCacheSize - numCachedTokens + leftPadLength;
